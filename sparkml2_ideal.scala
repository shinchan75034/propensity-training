...
val good_data = spark.sqlContext.sql("select mob_ban, cast(estimated_hh_income as int), cast(household_size as int), cast(number_adults as int), cast(number_children as int), cast(owner_renter as string), cast(home_property_type as string), webonly as response from kt6.acx_gold_2_vs")
...
val dfnumeric = indexStringColumns(good_data, Array("estimated_hh_income", "household_size", "number_adults", "number_children","owner_renter", "home_property_type", "response"))
...
val df_hot = oneHotEncodeColumns(dfnumeric, Array("estimated_hh_income", "household_size", "number_adults", "number_children","owner_renter", "home_property_type"))

// isolate features and target variables.
val va = new VectorAssembler().setOutputCol("features").setInputCols(df_hot.columns.diff(Array("response", "mob_ban")))

val lpoints = va.transform(df_hot).select("features", "response", "mob_ban").
      withColumnRenamed("response", "label")
...
val validpredicts = lrmodel.transform(adulttrain)
